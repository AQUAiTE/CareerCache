...
const tabs = [
  ...
  { label: "AI Assistant", icon: IconRobot, link: "/chat" },
];
...
export default function Navbar({ toggle }: NavbarProps) {
  ...
  const links = tabs.map((tab) =>
    tab.links ? (
      <LinksGroup {...tab} key={tab.label} onLinkClick={toggle} />
    ) : (
      <NavLink
        to={tab.link || "/"}
        className={`${classes.sololink} ${
          isActive(tab.link) ? classes.active : ""
        }`}
        key={tab.label}
      >
        <UnstyledButton
